  ...
  return `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;color:#2c2c2c;">
      ...
      <div style="padding:24px;">
        ...
        <p style="margin:0 0 20px;">Hi ${bookedBy}, a training session has been booked. Here are the details:</p>
...
module.exports = async function handler(req, res) {
  ...
  const extraEmails = additionalEmails
    ? additionalEmails.split(',').map(e => e.trim()).filter(e => e.includes('@'))
    : [];
  ...
  try {
    ...
    const html = buildConfirmationHtml({
      bookedBy, customerName, meetingTitle, startDateTime,
      trainerName: trainerName || 'Assigned trainer',
      rescheduleUrl, cancelUrl
    });
